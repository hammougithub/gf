...
var
  ...
  credits,credits_ini,cre_tot,cre_tot_nat,taux1,taux2,tep_nat,tem_nat,tmdm_nat,tmde_nat,dot_rev,dot_rev_nat
  ,annul1,annul2,mda,REIMP_ant:real;
implementation
 uses MAIN, uu_etat_credits, u_dm, u_etats,Unit_function_and_procedure,
  U_Parameters_etat_xxx, u_parameters_etat_x;
{$R *.dfm}

procedure Tf_etat_consommation.befor1(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
s1:='';
  qrlabel2.Caption:=' ? ? ? ? ?  ? ';
   if f_etats.SimpleDataSet_parameters1.locate('forme;champ',
                   vararrayof(['f_etat_consommation','qrlabel2']),[]) then
                    qrlabel2.Caption:=trim(f_etats.SimpleDataSet_parameters1.FieldByName('libellefr').AsString)
                    else begin
                        f_etats.SimpleDataSet_parameters1.Append;
                        f_etats.SimpleDataSet_parameters1.FieldByName('forme').AsString:='f_etat_consommation';
                        f_etats.SimpleDataSet_parameters1.FieldByName('champ').AsString:='qrlabel2';
                        f_etats.SimpleDataSet_parameters1.FieldByName('designation').AsString:='DIRECTION REGIONALE DU TRESOR  A GHARDAIA';
                        f_etats.SimpleDataSet_parameters1.FieldByName('libellefr').AsString:='DIRECTION REGIONALE DU TRESOR  A GHARDAIA';
                        f_etats.SimpleDataSet_parameters1.Post;s1:='x' ;
                    end;
if s1='x'  then  begin
                       f_Parameters_etat_x.DataSource1.DataSet:=f_etats.SimpleDataSet_parameters1;
...
procedure Tf_etat_consommation.BEFORE1(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
      QQ1.Caption:='-'+trim(c1.FieldByName('libelle_chapitre').AsString);
      cre_tot:=0.0;
        ...
        if  q_cre_affec.Fields[0].asstring<>'' then
         begin
           ...
           credits:=credits+q_cre_affec.Fields[0].value;
           credits_ini:=credits_ini+q_cre_affec.Fields[0].value;
       ...
       // if f_etats.cb1.Checked then begin
            ...
            if  q_enplus.Fields[0].asstring<>'' then
                  begin
                    ...
                    credits:=credits+q_enplus.Fields[0].value;
             ...
             if  q_enmoins.Fields[0].asstring<>'' then
                begin
                  ...
                  credits:=credits-q_enmoins.Fields[0].value;
      ...
      x:=0;
       ...
       // COLONNE DEPENSES ANTERIEURS
         ...
         // depenses annulees antérieures
           ...
           q_annul_ant.Params[2].Asdatetime:=strtodate('01/'+copy(f_etats.xdate.text,4,7),FormatSettings1);
           q_annul_ant.open;
          ...
          // REIMPUTATIONS ANTERIEURES
                ...
                q_reimp_ant.Params[2].Asdatetime:=strtodate('01/'+copy(f_etats.xdate.text,4,7),FormatSettings1);
           ...
           q_mda.Params[2].Asdatetime:=strtodate('01/'+copy(f_etats.xdate.text,4,7),FormatSettings1);
           q_mda.open;
           ...
           mda_tot:=mda_tot+mda;
           s:='            ';
           if mda<>0.0 then  s:=formater_float(mda); //str(mda:12:2,s); )
           qrmda.caption:=s+' ';
 ...
 // COLONNE DEPENSES DU MOIS
           ...
           q_annul_mois.Params[2].Asdatetime:=strtodate(f_etats.xdate.text,FormatSettings1);
           q_annul_mois.Params[3].Asdatetime:=strtodate('01/'+copy(f_etats.xdate.text,4,7),FormatSettings1);
           q_annul_mois.open;
          ...
          REIMP:=0.0; q_reimp_du_mois.close;
           ...
           q_reimp_du_mois.Params[2].Asdatetime:=strtodate(f_etats.xdate.text,FormatSettings1);
           q_reimp_du_mois.Params[3].Asdatetime:=strtodate('01/'+copy(f_etats.xdate.text,4,7),FormatSettings1);
         ...
         // MANDATS DU MOIS
           ...
           q_mdms.Params[2].Asdatetime:=strtodate(f_etats.xdate.text,FormatSettings1);
           q_mdms.Params[3].Asdatetime:=strtodate('01/'+copy(f_etats.xdate.text,4,7),FormatSettings1);
           q_mdms.open;
           ...
           s:='            ';
           if mdm<>0.0 then s:=formater_float(mdm);// str(mdm:12:2,s);
           qrmdm.caption:=s+' ';
 ...
 // COLONNE  TOTAL DEPENSES
        ...
        if  q_reimp.Fields[0].asstring<>'' then reimp:=q_reimp.Fields[0].value;
         ...
         // depenses annulees
           ...
           q_annul.Params[2].Asdatetime:=strtodate(f_etats.xdate.text,FormatSettings1);
           q_annul.open;
       ...
       // tout les MANDATS
        ...
        if  q_mdm.Fields[0].asstring<>'' then  tot_dep:=q_mdm.Fields[0].value;
           ...
           s:='            ';
           if tot_dep<>0.0 then s:=formater_float(tot_dep);// str(tot_dep:12:2,s);
           qrltot_dep.caption:=s+' ';

             s1:='     ';

            if (cre_tot<>0.0) and (tot_dep<>0.0) then begin
...
begin
  ...
  mda_nat:=0.0; tot_dep_nat:=0.0;credits:=0.0;;credits_ini:=0.0
      //  s1:='';
...
begin
 ...
 if (dot_rev_nat<>0) or (tot_dep_nat<>0) then s:=formater_float(dot_rev_nat-tot_dep_nat);qrlsolde2.caption:=s+' ';
   s1:='     ';
   if (dot_rev_nat<>0.0) and (tot_dep_nat<>0) then begin
               taux2:=(tot_dep_nat/dot_rev_nat*100);
               str(taux2:5:2,s1);
                                 end;
   qrltaux2.caption:=s1+' ';
...
procedure Tf_etat_consommation.befort7(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
   s:='            ';if credits_ini<>0 then s:=formater_float(credits_ini);qr1xx.Caption:=s+' ';
   ...
   if (dot_rev-tot_dep_tot)<>0 then s:=formater_float(dot_rev-tot_dep_tot);qrlsolde22.caption:=s+' ';
   s1:='     ';
   if (dot_rev<>0.0) and (tot_dep_tot<>0) then begin
               taux2:=(tot_dep_tot/dot_rev*100);
               str(taux2:5:2,s1);
                                 end;
