unit U_f_article_etat_tout_agent;
...
begin
  if edit1.text<>'' then
     f_dm.simpledataset_designation.locate('designation',trim(edit1.text), [loCaseInsensitive,loPartialKey]);
end;
...
begin
     ...
     lignes.Params[0].AsString:=f_dm.SimpleDataSet_designation.FieldByName('ref').asstring;
     lignes.open;Cl1.Open;DBGRID3.Refresh;
     label2.Caption:= inttostr(Cl1.RecordCount)+'-- lignes -- '+trim(f_dm.SimpleDataSet_designation.FieldByName('designation').asstring)+'     '+DateTimeToStr(Now);
end;
...
begin
      if edit3.text<>'' then
     f_dm.simpledataset_designation.locate('ref',trim(edit3.text), [loCaseInsensitive,loPartialKey]);
end;
