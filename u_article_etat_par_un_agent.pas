unit u_article_etat_par_un_agent;
...
uses
  ...
  Data.SqlExpr, Vcl.Grids, Vcl.DBGrids,printers;

type
  Tf_article_etat_par_un_agent = class(TForm)
    DBGrid1: TDBGrid;
    lignes: TSQLQuery;
    cl1: TClientDataSet;
...
var
  f_article_etat_par_un_agent: Tf_article_etat_par_un_agent;

implementation

{$R *.dfm}

uses u_forme1,u_dm, u_consommation, U_article_etat_tout_agent;

procedure Tf_article_etat_par_un_agent.Button2Click(Sender: TObject);
begin
...
procedure Tf_article_etat_par_un_agent.des_find(Sender: TObject);
begin
     if edit1.text<>'' then
     f_dm.clientdataset_desig.locate('designation',trim(edit1.text), [loCaseInsensitive,loPartialKey]);
end;

procedure Tf_article_etat_par_un_agent.filtrer(Sender: TObject);
begin
     ...
     lignes.Params[1].AsString:=f_dm.SimpleDataSet_agent.FieldByName('mat').asstring;
     lignes.open;Cl1.Open;DBGRID3.Refresh;
     label2.Caption:= inttostr(Cl1.RecordCount)+'-- lignes -- '+trim(f_dm.clientdataset_desig.FieldByName('designation').asstring)
     +'  '+trim(f_dm.SimpleDataSet_agent.FieldByName('noma').AsString)
     +' '+trim(f_dm.SimpleDataSet_agent.FieldByName('prenoma').AsString)+'  '+datetostr(Now);
end;


procedure Tf_article_etat_par_un_agent.mat_find(Sender: TObject);
begin
   if mat.text<>'' then
    if (f_dm.simpledataset_agent.locate('mat',trim(mat.text),[])) then dbgrid4.REFRESH;
end;

procedure Tf_article_etat_par_un_agent.name_find(Sender: TObject);
begin
...
procedure Tf_article_etat_par_un_agent.ref_find(Sender: TObject);
begin
    if edit3.text<>'' then
     f_dm.clientdataset_desig.locate('ref',trim(edit3.text), [loCaseInsensitive,loPartialKey]);
end;

procedure Tf_article_etat_par_un_agent.SpeedButton1Click(Sender: TObject);
const
  LeftMargin = 0.05;
  TopMargin = 0.05;
  BottomMargin = 0.05;
begin
  ...
  printDialog := TPrintDialog.Create(f_article_etat_par_un_agent);
  ...
  if printDialog.Execute then
    begin
       ...
       // x := Round(LeftMargin*Printer.PageWidth);
        ...
        y := y + printer.Canvas.TextHeight('A')+3;
        printer.Canvas.TextOut(1,y,'             Date          QuantitÈ                                        Observation');
        y := y + printer.Canvas.TextHeight('A')+3;
         printer.Canvas.TextOut(1,y,'------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------');
        y := y + printer.Canvas.TextHeight('A')+3;
        while not DBGrid3.DataSource.DataSet.Eof do
          begin
            x := Round(LeftMargin*Printer.PageWidth);
             printer.Canvas.TextOut(x,y,datetostr(DBGrid3.DataSource.DataSet.Fields[0].Asdatetime));
               x := x + MulDiv(DBGrid3.Columns[0].Width,Printer.PageWidth, DBGrid3.Width);
             printer.Canvas.TextOut(x,y,floattostrf(DBGrid3.DataSource.DataSet.Fields[1].Asfloat,ffNumber,12,2));
...
procedure Tf_article_etat_par_un_agent.tri(Sender: TObject);
begin
